...
cat > benchmarks/trillionx_codespaces_workspace_bench.js <<'JS'
"use strict";
...
node --check benchmarks/trillionx_codespaces_workspace_bench.js
node benchmarks/trillionx_codespaces_workspace_bench.js

git add TRILLIONX_CODESPACES_WORKSPACE_BENCH.sh \
  benchmarks/trillionx_codespaces_workspace_bench.js \
  reports/TRILLIONX_CODESPACES_WORKSPACE_BENCH_LATEST.json \
  history/TRILLIONX_CODESPACES_WORKSPACE_BENCH_HISTORY.jsonl 2>/dev/null || true

git commit -m "Add TRILLIONX Codespaces workspace benchmark" || echo "Rien à commit"

echo "✅ Bench terminé"
echo "Rapport: reports/TRILLIONX_CODESPACES_WORKSPACE_BENCH_LATEST.json"
