...
echo "============================================================"
echo " TRILLIONX ASYNC DRAIN + REGISTRY + CODEC + DERIVATIVES"
echo "============================================================"

mkdir -p scripts logs reports history runtime_state async_queue async_jobs async_results async_registry async_codecs async_derivatives

cat > scripts/tx_async_drain_registry_codec.js <<'JS'
"use strict";
...
cat > scripts/tx_async_drain_daemon_stop.sh <<'SH'
#!/usr/bin/env bash
cd /workspaces/TRILLIONX || exit 1
[ -f runtime_state/TRILLIONX_ASYNC_DRAIN_DAEMON_PID ] && kill "$(cat runtime_state/TRILLIONX_ASYNC_DRAIN_DAEMON_PID)" 2>/dev/null || true
...
cat > scripts/tx_async_submit2.sh <<'SH'
#!/usr/bin/env bash
cd /workspaces/TRILLIONX || exit 1
TYPE="${1:-status}"
...
chmod +x scripts/tx_async_drain*.sh scripts/tx_async_submit2.sh scripts/tx_async_clear_old_results.sh scripts/tx_async_catalog.sh

echo "=== ALIAS ASYNC DRAIN ==="
cat >> .trillionx_aliases <<'ALIAS'

alias txdrain='cd /workspaces/TRILLIONX && bash scripts/tx_async_drain.sh'
alias txds='cd /workspaces/TRILLIONX && bash scripts/tx_async_drain_status.sh'
alias txdd='cd /workspaces/TRILLIONX && bash scripts/tx_async_drain_daemon_start.sh'
alias txddstop='cd /workspaces/TRILLIONX && bash scripts/tx_async_drain_daemon_stop.sh'
alias txsub='cd /workspaces/TRILLIONX && bash scripts/tx_async_submit2.sh'
alias txcat='cd /workspaces/TRILLIONX && bash scripts/tx_async_catalog.sh'
ALIAS

echo "=== TEST LOCAL ==="
node scripts/tx_async_drain_registry_codec.js status >/tmp/tx_async_status_test.json
python3 -m json.tool /tmp/tx_async_status_test.json >/dev/null && echo "JSON STATUS OK"

echo "=== PREMIER DRAIN ==="
TX_ASYNC_MAX_JOBS=20 TX_ASYNC_STALE_MS=60000 node scripts/tx_async_drain_registry_codec.js drain | tee reports/TRILLIONX_ASYNC_DRAIN_FIRST_RUN.txt

git add scripts/tx_async_* .trillionx_aliases \
  async_registry/TRILLIONX_ASYNC_JOB_REGISTRY.json \
  async_codecs/TRILLIONX_ASYNC_CODEC_CATALOG.json \
  async_derivatives/TRILLIONX_ASYNC_DERIVATIVE_CATALOG.json \
  reports/TRILLIONX_ASYNC_DRAIN_REGISTRY_CODEC_LATEST.json \
  reports/TRILLIONX_ASYNC_DRAIN_FIRST_RUN.txt \
  history/TRILLIONX_ASYNC_DRAIN_REGISTRY_CODEC_HISTORY.jsonl 2>/dev/null || true

git commit -m "Add TRILLIONX async drain registry codec derivatives" || echo "Rien à commit"

echo "============================================================"
echo "✅ ASYNC DRAIN REGISTRY CODEC OK"
echo "Status     : bash scripts/tx_async_drain_status.sh"
echo "Drain      : TX_ASYNC_MAX_JOBS=50 bash scripts/tx_async_drain.sh"
echo "Daemon     : bash scripts/tx_async_drain_daemon_start.sh"
echo "Stop daemon: bash scripts/tx_async_drain_daemon_stop.sh"
echo "Submit     : bash scripts/tx_async_submit2.sh status"
echo "Catalogue  : bash scripts/tx_async_catalog.sh"
echo "Clear old  : bash scripts/tx_async_clear_old_results.sh"
echo "============================================================"
