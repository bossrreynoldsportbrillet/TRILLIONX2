...
mkdir -p benchmarks reports history logs

cat > benchmarks/trillionx_system_organization_bench.js <<'JS'
"use strict";
...
node --check benchmarks/trillionx_system_organization_bench.js
node benchmarks/trillionx_system_organization_bench.js

git add TRILLIONX_SYSTEM_ORGANIZATION_BENCH.sh \
  benchmarks/trillionx_system_organization_bench.js \
  reports/TRILLIONX_SYSTEM_ORGANIZATION_BENCH_LATEST.json \
  history/TRILLIONX_SYSTEM_ORGANIZATION_BENCH_HISTORY.jsonl 2>/dev/null || true

git commit -m "Add TRILLIONX system organization benchmark" || echo "Rien à commit"

echo "✅ Organisation bench terminé"
echo "Rapport: reports/TRILLIONX_SYSTEM_ORGANIZATION_BENCH_LATEST.json"
