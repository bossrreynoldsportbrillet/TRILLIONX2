...
function req(url){
 return new Promise(res=>{
  ...
  const q=lib.get(url,{timeout:TIMEOUT,headers:{"user-agent":"TRILLIONX-network-connections-total"}},rsp=>{
   let n=0;
...
function interfaces(){
 ...
 for(const [name,arr] of Object.entries(n)){
  for(const x of arr||[]){
   out.push({
    name,
    family:x.family,
...
function parsePorts(raw){
 ...
 for(const line of raw.split("\n").filter(Boolean)){
  let m=line.match(/(?:LISTEN|UNCONN).*?((?:127\.0\.0\.1|0\.0\.0\.0|\[::\]|::1|\*|[0-9a-fA-F:.]+):([0-9]+))/);
  if(!m)m=line.match(/[:.]([0-9]{2,5})\s/);
  if(m){
   const port=Number(m[2]||m[1]);
   if(port)ports.push({port,line});
  }
 }
 return [...new Map(ports.map(x=>[x.port,x])).values()].sort((a,b)=>a.port-b.port);
}

function runtimeNetwork(){
 const listeningRaw=sh("ss -lntup 2>/dev/null || netstat -lntup 2>/dev/null || lsof -i -P -n 2>/dev/null",7000);
 const activeRaw=sh("ss -tunap 2>/dev/null | head -250 || netstat -tunap 2>/dev/null | head -250 || lsof -i -P -n 2>/dev/null | head -250",7000);
 return {
  ...
  listening_raw:listeningRaw.split("\n").slice(0,250),
  active_raw:activeRaw.split("\n").slice(0,250),
  listening_ports:parsePorts(listeningRaw)
 };
}
...
function repoScan(){
 ...
 for(const f of files){
  ...
  re=/\/api\/[A-Za-z0-9_\-./:]+/g;
  while((m=re.exec(s)))api.push({api:m[0],file:f});
  re=/(io|socket|ws|server)\.(on|emit)\s*\(\s*['"`]([^'"`]+)/g;
  while((m=re.exec(s)))sockets.push({obj:m[1],op:m[2],event:m[3],file:f});
  re=/https?:\/\/[A-Za-z0-9._~:/?#\[\]@!$&'()*+,;=%-]+/g;
  while((m=re.exec(s)))urls.push({url:m[0].slice(0,220),file:f});
  re=/\b(PORT|port)\b\s*[:=]\s*([0-9]{2,5})|\blisten\s*\(\s*([0-9]{2,5})/g;
  while((m=re.exec(s)))ports.push({port:Number(m[2]||m[3]),file:f,match:m[0]});
 }
 const apiRoots={};
 for(const a of api){const k=a.api.split("/").slice(0,3).join("/");apiRoots[k]=(apiRoots[k]||0)+1}
 const urlHosts={};
 for(const u of urls){
  try{const h=new URL(u.url).hostname;urlHosts[h]=(urlHosts[h]||0)+1}catch{}
 }
 return {
  files:files.length,
  routes:routes.length,
  api_strings:api.length,
  socket_events:sockets.length,
  urls:urls.length,
  ...
  sample_routes:routes.slice(0,160),
...
async function dnsProbe(){
 const hosts=[
  ...
  "cloudflare.com","google.com","one.one.one.one"
 ];
 const out=[];
 for(const h of hosts){
  ...
  try{
   ...
   out.push({host:h,ok:true,ms:r(performance.now()-t0),address:a.address,family:a.family});
  }catch(e){
   out.push({host:h,ok:false,ms:r(performance.now()-t0),error:e.code||e.message});
  }
 }
 return out;
}
...
async function externalProbe(){
 ...
 const out=[];
 for(const u of urls)out.push(await req(u));
 return out;
}

(async()=>{
 console.log("=== TRILLIONX NETWORK CONNECTIONS TOTAL ===");
 ...
 const net=runtimeNetwork();
 const repo=repoScan();
 const declaredPorts=repo.declared_ports.map(x=>x.port).filter(Boolean);
 const listeningPorts=net.listening_ports.map(x=>x.port);
 const ports=[...new Set([3000,8080,8000,5000,5173,...declaredPorts,...listeningPorts])].filter(Boolean).sort((a,b)=>a-b);
 ...
 const okDns=dns.filter(x=>x.ok).length;
 const okExt=external.filter(x=>x.ok).length;
 const lat=[...local,...external].filter(x=>x.ok).map(x=>x.ms).sort((a,b)=>a-b);
 const p50=lat.length?lat[Math.floor(lat.length*0.50)]:null;
 const p95=lat.length?lat[Math.floor(lat.length*0.95)]||lat.at(-1):null;

 let health=100;
 health -= (local.length-okLocal)*1.5;
 health -= (dns.length-okDns)*3;
 health -= (external.length-okExt)*4;
 if(p95&&p95>1500)health-=10;
 if(!ports.includes(3000))health-=15;
 health=Math.max(0,r(health));

 const report={
  engine:"TRILLIONX_NETWORK_CONNECTIONS_TOTAL",
  ts:new Date().toISOString(),
  policy:{
   target:"TRILLIONX",
   host:"CODESPACES_SUPPORT_ONLY",
   auto_detect_all_connections:true,
   auto_detect_ports:true,
   auto_detect_repo_network_surface:true,
  ...
  summary:{
   ...
   p95_ms:p95,
   health,
   verdict:health>=85?"NETWORK_CONNECTIONS_TOTAL_GOOD":health>=65?"NETWORK_CONNECTIONS_TOTAL_PARTIAL":"NETWORK_CONNECTIONS_REVIEW_NEEDED"
  }
 };

 const file=`data/trillionx_network_connections_total_${Date.now()}.json`;
 fs.writeFileSync(file,JSON.stringify(report,null,2));
 fs.writeFileSync("data/trillionx_network_connections_total_latest.json",JSON.stringify(report,null,2));

 console.log("INTERFACES:",report.summary.interfaces);
 console.log("PORTS:",ports.join(",")||"none");
 console.log("LISTENING:",listeningPorts.join(",")||"none");
