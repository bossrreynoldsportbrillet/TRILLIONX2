...
echo "============================================================"
echo " TRILLIONX PARALLEL μ-PACKET SYNC PRIORITY"
echo "============================================================"

mkdir -p scripts logs reports history runtime_state micro_sync

cat > scripts/tx_parallel_micro_sync.js <<'JS'
"use strict";
...
node --check scripts/tx_parallel_micro_sync.js

cat > scripts/tx_parallel_micro_sync_start.sh <<'SH'
#!/usr/bin/env bash
cd /workspaces/TRILLIONX || exit 1
...
cat > scripts/tx_parallel_micro_sync_stop.sh <<'SH'
#!/usr/bin/env bash
cd /workspaces/TRILLIONX || exit 1
[ -f runtime_state/TRILLIONX_PARALLEL_MICRO_SYNC_PID ] && kill "$(cat runtime_state/TRILLIONX_PARALLEL_MICRO_SYNC_PID)" 2>/dev/null || true
...
cat > scripts/tx_parallel_micro_sync_status.sh <<'SH'
#!/usr/bin/env bash
cd /workspaces/TRILLIONX || exit 1
...
chmod +x scripts/tx_parallel_micro_sync_*.sh

node - <<'NODE'
const fs=require("fs"),crypto=require("crypto");
const rep={engine:"TRILLIONX_PARALLEL_MICRO_SYNC_PRIORITY_INSTALL",time:new Date().toISOString(),status:"READY",safe:true,parallel:true,priority:"nice -5 if permitted, fallback normal",does_not_touch:["app.js","data","raid60_plus","node_modules"]};
rep.seal=crypto.createHash("sha256").update(JSON.stringify(rep)).digest("hex");
fs.writeFileSync("reports/TRILLIONX_PARALLEL_MICRO_SYNC_PRIORITY_INSTALL.json",JSON.stringify(rep,null,2));
console.log(JSON.stringify(rep,null,2));
NODE

git add scripts/tx_parallel_micro_sync* scripts/trillionx_parallel_micro_sync_priority.sh reports/TRILLIONX_PARALLEL_MICRO_SYNC_PRIORITY_INSTALL.json 2>/dev/null || true
git commit -m "Add TRILLIONX parallel priority micro packet sync" || echo "Rien à commit"

echo "============================================================"
echo "✅ PARALLEL μ-PACKET PRIORITY SYNC INSTALLE"
echo "Démarrer : TX_MICRO_SYNC_WORKERS=4 TX_MICRO_SYNC_MS=250 bash scripts/tx_parallel_micro_sync_start.sh"
echo "Statut   : bash scripts/tx_parallel_micro_sync_status.sh"
echo "Stop     : bash scripts/tx_parallel_micro_sync_stop.sh"
echo "============================================================"
